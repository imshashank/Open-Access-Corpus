	...
	#publisher_article table
		...
		#tags_table
			...
			for concept in concepts:
				...
				q = "Select count(*) from `corpus`.`concepts` where concept_name like '%{0}%' LIMIT 1;".format(concept)
				print q
				cur.execute(q)
				for row in cur.fetchall():
					count= row[0]
			...
			for concept in concepts:
				...
				concept = MySQLdb.escape_string(concept.decode('latin1').encode('utf8').rstrip())
				#concept = MySQLdb.escape_string(concept.encode('utf-8').rstrip())
				q = "Select concept_id from `corpus`.`concepts` where concept_name like '%{0}%' LIMIT 1 ;".format(concept)
				## print q
				cur.execute(q)
				for row in cur.fetchall():
					concept_id= row[0]
				...
				q = "Select count(*) from `corpus`.`article_concept` where alternate_id like '{0}' and concept_id like '{1}' LIMIT 1 ;".format(alternate_id,concept_id)
				cur.execute(q)
				for row in cur.fetchall():
					count= row[0]
					break
