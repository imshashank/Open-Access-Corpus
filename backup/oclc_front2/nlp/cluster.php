...
$tset->addDocument(
    ...
    new TokensDocument(array('x'=>$row['article_id'],'y'=>$row['tag_id']))
);
}
...
list($dendrogram) = $clust->cluster($tset, new DataAsFeatures());
 
//print_r($dendrogram);
$cl = Hierarchical::dendrogramToClusters($dendrogram, 3);
...
$im = drawClusters(
            ...
            null, // no centroids
            true, // no lines
            5 // emphasize points (for little points)
        );

var_dump($cl);
//print_r(Hierarchical::dendrogramToClusters($dendrogram, 5));
$dg = drawDendrogram(
            $tset,
            $dendrogram,
            10600, // width
...
$rows=[];
foreach ($cl as $value){
$rows[]= $obj->get_tag_name($value);
}
*/
//var_dump($rows);

function drawDendrogram($tset, $dendrogram, $w=300, $h=3600){
    ...
    function drawClusters($tset, $clusters, $centroids=null, $lines=True,$emphasize=1,$w=2300,$h=2300)
    {
        if (!function_exists('imagecreate'))
            return null;
